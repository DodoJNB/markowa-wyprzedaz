...
Chatbot (bąbelek w prawym dolnym rogu strony) korzysta z **Google Gemini API** (model Gemini 2.5 Flash) — całkowicie za darmo, bez podawania karty płatniczej. Darmowy limit to ok. 1500 zapytań dziennie, co dla FAQ-bota na stronie streamera to bardzo duży zapas.

1. Wejdź na [aistudio.google.com](https://aistudio.google.com) i zaloguj się kontem Google.
2. Kliknij **„Get API key"** → **„Create API key"**.
3. Skopiuj wygenerowany klucz.
4. W Netlify dodaj zmienną środowiskową:
   - `GEMINI_API_KEY` = twój klucz (zaznacz "Contains secret values")
5. Zrób redeploy.
